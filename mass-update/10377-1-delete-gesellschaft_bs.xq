...
declare function _:select-entries() as element(mods:mods)* {
  (: Suche "Gesellschaft" vor "Bildung und Erziehung" :)
  collection($__db__)//mods:mods[.//mods:topic[. eq 'Gesellschaft'][ancestor::mods:subject[@usage='secondary'][mods:topic='Bildung und Erziehung']]] 
};
...
declare %updating function _:transform($e as element(mods:mods)) {
  for $topic in $e//mods:topic[. eq 'Gesellschaft'][ancestor::mods:subject[@usage='secondary'][mods:topic='Bildung und Erziehung']]
     return (
       insert node comment{'task #10377,1: deleted topic:Gesellschaft'} before $topic,
       delete node $topic
     )
};
