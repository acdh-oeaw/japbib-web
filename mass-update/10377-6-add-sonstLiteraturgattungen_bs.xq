...
declare function _:select-entries() as element(mods:mods)* {  
  collection($__db__)//mods:mods[.//mods:topic[matches(., '^Literaturgattung$')]]
}; 
...
declare %updating function _:transform($e as element(mods:mods)) {
  let $newTopic := (    
    <!--Task #10377,6: add sonstige Literaturgattungen-->,
    <topic>sonstige Literaturgattungen</topic>
    ) 
  for $topic in $e//.//mods:topic[matches(., '^Literaturgattung$')]
      return replace node $topic with $newTopic
};
