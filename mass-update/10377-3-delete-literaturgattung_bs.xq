...
declare variable $_:maxNumberOfChangesPerJob external := 15000;
...
declare function _:select-entries() as element(mods:mods)* { 
  collection($__db__)//mods:mods[.//mods:topic[. eq 'Literaturgattung'][following-sibling::mods:topic[matches(., 'Hochschulschrift|Rezension|Übersetzung')]]]
}; 
...
declare %updating function _:transform($e as element(mods:mods)) {
  for $topic in $e//mods:topic[. eq 'Literaturgattung'][following-sibling::mods:topic[matches(., 'Hochschulschrift|Rezension|Übersetzung')]]
     return (
       insert node comment{'task #10377,3,: deleted topic:Literaturgattung'} before $topic,
       delete node $topic
     )
};
