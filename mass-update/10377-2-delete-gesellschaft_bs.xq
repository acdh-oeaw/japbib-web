...
declare function _:select-entries() as element(mods:mods)* {
  (: Suche alles, was zu "Bildung und Erziehung" automatisch hinzugefügt wurde :)
  collection($__db__)//mods:mods[.//mods:topic[. eq 'Gesellschaft'][ancestor::mods:mods[mods:subject[@usage='primary'][mods:topic='Bildung und Erziehung']]][ancestor::mods:subject[@usage='secondary'][not(mods:topic[position()>2])]]]
}; 
...
declare %updating function _:transform($e as element(mods:mods)) {
  for $topic in $e//mods:topic[. eq 'Gesellschaft'][ancestor::mods:mods[mods:subject[@usage='primary'][mods:topic='Bildung und Erziehung']]][ancestor::mods:subject[@usage='secondary'][not(mods:topic[position()>2])]]
     return (
       insert node comment{'task #10377,2: deleted topic:Gesellschaft'} before $topic,
       delete node $topic
     )
};
