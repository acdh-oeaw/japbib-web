...
declare function _:select-entries() as element(mods:mods)* { 
  ...
  collection($__db__)//mods:mods[.//mods:subject[@usage='primary'][mods:topic[matches(., 'Hochschulschrift|Rezension|Übersetzung')]]/following-sibling::mods:subject[1][@usage='secondary']/mods:topic[. eq 'Literaturgattung']]
}; 
...
declare %updating function _:transform($e as element(mods:mods)) {
  for $topic in $e//.//mods:subject[@usage='primary'][mods:topic[matches(., 'Hochschulschrift|Rezension|Übersetzung')]]/following-sibling::mods:subject[1][@usage='secondary']/mods:topic[. eq 'Literaturgattung']
     return (
       insert node comment{'task #10377,4,: deleted topic:Literaturgattung'} before $topic,
       delete node $topic
     )
};
