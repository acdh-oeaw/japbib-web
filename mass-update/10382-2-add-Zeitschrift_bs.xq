...
declare function _:select-entries() as element(mods:mods)* {
  collection($__db__)//mods:mods[mods:subject[@usage eq 'primary' and mods:topic[matches(., '^Zeitschrift$')]] and not(mods:genre)]
};
...
declare %updating function _:transform($e as element(mods:mods)) {
      let $type := $e/mods:typeOfResource, 
         ...
         Journal {comment {'#Task #10382,2 add Journal'}}        
         </genre>
         ) 
