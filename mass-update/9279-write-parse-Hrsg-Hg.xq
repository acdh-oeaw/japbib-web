...
declare function _:transform($e as element(mods:mods)) as element(mods:mods) {
  ...
  { for $name in .//mods:name[matches(mods:namePart, $_:regex)]
    ...
    return (replace node $name with _:createName($name/mods:namePart, $nns[1]/text(), $vns[1]/text(), $mns[1]/text()),
            if ($nns[2]/text()) then insert node _:createName($name/mods:namePart, $nns[2]/text(), $vns[2]/text(), $mns[2]/text()) after $name else (),
            if ($nns[3]/text()) then insert node _:createName($name/mods:namePart, $nns[3]/text(), $vns[3]/text(), $mns[3]/text()) after $name else ())}
};
