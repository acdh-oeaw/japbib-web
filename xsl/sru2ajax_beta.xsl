...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
 ...
 Co-Autoren -->
    ...
    <xsl:template name="full-Dateooo" match="//mods:dateIssued">
        <xsl:param name="date" />
        <xsl:param name="keyDate"/> 
        <xsl:choose>
            <xsl:when test="//mods:dateIssued[matches(., '^\d{4}')]">
        <xsl:for-each select="//mods:dateIssued">
            <xsl:choose> 
                <xsl:when test=".[@encoding = 'iso8601'] and $date ne 'year' and $keyDate ne 'yes'">
                ...
                <xsl:when test="$date eq 'year'">
                    <xsl:value-of select="./substring(., 1, 4)"/>
                </xsl:when>
                <xsl:when test="$keyDate eq 'yes'">
                    <xsl:value-of select=".[@keyDate eq 'yes']"/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:value-of select="."/>
                </xsl:otherwise>
    ...
    <xsl:template match="mods:dateIssued" name="full-Date">
        <xsl:value-of select="mods:dateIssued/substring(., 1, 4)"/> 
    </xsl:template>
    ...
    <xsl:template match="mods:originInfo[ancestor::mods:mods/mods:genre[@authority='local'][matches(., 'journalArticle|newspaperArticle')] 
        ...
        <li><a href="#?query=title=&quot;{../mods:titleInfo/mods:title}&quot;" class="stichwort"><xsl:value-of select="../mods:titleInfo/mods:title"/></a> 
            <xsl:value-of select="if (../mods:part/mods:detail[@type eq 'volume']) then ', '||_:dict('volumeJournal')||' '||../mods:part/mods:detail[@type eq 'volume']/mods:number else ''"/><xsl:choose>
                ...
                <xsl:when test="mods:dateIssued">                    
                    <xsl:value-of select="', '"/><xsl:call-template name="full-Date"/>
                </xsl:when>
                <xsl:otherwise>
